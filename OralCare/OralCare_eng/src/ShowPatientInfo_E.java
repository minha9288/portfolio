...
public class ShowPatientInfo_E {

	public ShowPatientInfo_E(String path){
...
//	File file = new File("C:\\Users\\pc\\Desktop\\patientInfo.xls");
	...
	PatientInfoBean_E input_bean = new PatientInfoBean_E();
	JTextField input;
	...
	public JPanel SearchTab() {
		JLabel label_search = new JLabel("Input Chart-Num : ");
	...
	public ScrollPane PatientInfo() {
		...
		pGender.add(rbMan);
		pGender.add(rbWoman);
		if(input_bean.getGender()=="Male")
			rbMan.setSelected(true);
		else
			rbWoman.setSelected(true);

		JPanel pC = new JPanel();
		JLabel CL = new JLabel("if this information is true, press OK button");
	...
	public void readPatient(){
		...
		if(!file.isFile()){
			JOptionPane.showMessageDialog(null, "no files", "warning", JOptionPane.WARNING_MESSAGE);
		}else{
		...
		try{
			...
			for(int i=1; i<rowCount; i++){
				...
				if(sheet.getCell(0,i).getContents().toString().equals(input_chartNum)){
					...
					input_bean = new PatientInfoBean_E();
					...
					if(input_bean.getGender().equals("Male"))
						rbMan.setSelected(true);
					else
						rbWoman.setSelected(true);
					invalidate();
