...
public class Settings {
    public static String ip; // RemoteDroid에서 받아온 ip 저장하는 변수
    ...
    //
    private static final String PREFS_FILENAME = "MultiMouse";
    ...
    public static void setIp(String ip) throws Exception {
        /*SharedPreferences는 안드로이드에서 간단한 값을 저장하는 경우에 많이 쓰임
          운영체제가 어플리케이션을 메모리상의 이유로 강제종료시키더라도,
          다시 실행시켰을 때 자신이 사용하던 화면이 나오게 하기 위해 도와주는 것.
          간단하게, UI의 상태를 저장하는 클래스 XML파일 형식으로 저장
          기능에는 1.데이터를 저장하는 기능. 2. 저장한 데이터를 불러오는 기능.
        */
        SharedPreferences.Editor edit = prefs.edit();

        // testIPVaild 함수 호출, ip주소가 맞는지 확인
        testIPValid(ip);
        // 데이터 저장
    ...
    private static void testIPValid(String ip) throws Exception {
        try {
            // ip를 점으로 나눠 배열 octets에 한칸 한칸 저장!
