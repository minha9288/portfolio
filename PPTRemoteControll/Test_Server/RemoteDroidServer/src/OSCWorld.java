...
public class OSCWorld extends World {
        ...
        public void onEnter() {
                try {
                        ...
                        this.receiver.addListener("/test", listener);
                    
                        //이전페이지 이동
                        listener = new OSCListener() {
                            public void acceptMessage(java.util.Date time, OSCMessage message) {
                            	System.out.println("이전페이지 들어옴");
                            	prevbuttonEvent();                          	
                            }
                        };
                        this.receiver.addListener("/prevbutton", listener);
                        
                        //다음페이지 이동
                        listener = new OSCListener() {
                            public void acceptMessage(java.util.Date time, OSCMessage message) { 
                            	System.out.println("다음페이지 들어옴");
                            	nextbuttonEvent();                          	
                            }
                        };
                        this.receiver.addListener("/nextbutton", listener);
                        ...
                        listener = new OSCListener() {
                            public void acceptMessage(java.util.Date time, OSCMessage message) {
                            	//사용자가 선택한 페이지 값 받아오기
                            	Object[] args = message.getArguments();
                                String selectPage = args[0].toString();
                                
                            	gotobuttonEvent(selectPage);                          	
                            }
                        };
                        this.receiver.addListener("/gotobutton", listener);
        ...
        private void testEvent() {
        	//this.keyPress(KeyEvent.VK_CONTROL, KeyEvent.VK_P);  //ctrl+p : 싸인펜 모드
       	//this.keyPress(KeyEvent.VK_F5);  //F5 : 쇼시작
       	this.keyPress(KeyEvent.VK_CONTROL);
        }
        
        private void prevbuttonEvent() {
        	System.out.println("이벤트들어옴1");
        	this.keyPress(KeyEvent.VK_BACK_SPACE);
        	//this.keyPress(KeyEvent.VK_P);
        }
        
        private void nextbuttonEvent() {
        	System.out.println("이벤트들어옴2");
        	this.keyPress(KeyEvent.VK_ENTER);
        }
        
        private void gotobuttonEvent(String pageNumber) {
        	String keycode = "KeyEvent.VK_" + pageNumber + "";
        	this.keyPress(Integer.parseInt(keycode), KeyEvent.VK_ENTER);
        }
       ...
       /**
        ...
        //단축키받기
        private void keyPress(int localcode, int localcode2) {  	 
            try {
                    this.robot.keyPress(localcode);
                    this.robot.keyPress(localcode2);
            } catch (IllegalArgumentException e) {
                    System.out.println("Invalid keyPress code: " + localcode);
            }
    }
