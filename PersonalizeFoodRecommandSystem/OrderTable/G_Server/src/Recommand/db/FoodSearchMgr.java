...
public class FoodSearchMgr {
	 ...
	 * 10개받아오는거
	 */
	public ArrayList<Food> getMenuFood(String c_user_nation){
		Connection con = null;
		...
		try {	
			...
			while(rs.next()){
				...
				menu_list.add(bean);
				System.out.println(bean.getId() + " / " + bean.getName() + " / " + bean.getMaterialList().get(0).getId() + " / " + bean.getMaterialList().get(1).getId());
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
