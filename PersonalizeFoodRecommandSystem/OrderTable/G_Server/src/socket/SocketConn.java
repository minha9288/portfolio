			...
			while(true){
				...
				for(Iterator iterator=key.iterator(); iterator.hasNext();){
					...
					if(keyName.equals("user")){
						...
						//모바일에서 user를 보내면 얘가 sendFood를 함
						//sendfood에서의 키네임이 food이기에 모바일에서 받음
						SendFood();
					...
					System.out.println("키네임은="+keyName);
					
					if(keyName.equals("selected_food")){
						
						//모바일에서 selected_food를 보내면 얘가 sendselectedfood를 함
