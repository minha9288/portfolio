...
public class BoardMgr {
	...
	private static final String  SAVEFOLDER = "D:/JSP/workspace1/project/WebContent/boardfile";
	...
	public Vector<BoardBean> getBoardList(String keyField, String keyWord,
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select * from bbs order by ref desc, pos limit ?, ?";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			con = pool.getConnection();
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(num) from bbs";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(num) from  bbs where " + keyField + " like ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
			}
			rs = pstmt.executeQuery();
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert bbs(name,content,subject,ref,pos,depth,regdate,pass,count,ip,filename,filesize)";
	...
	public void upCount(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "update bbs set count=count+1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void deleteBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select filename from bbs where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public void updateBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update bbs set name=?,subject=?,content=? where num=?";
			pstmt = con.prepareStatement(sql);
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert bbs (name,content,subject,ref,pos,depth,regdate,pass,count,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
	...
	public void replyUpBoard(int ref, int pos) {
		...
		try {
			con = pool.getConnection();
			sql = "update bbs set pos = pos + 1 where ref=? and pos > ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(2, pos);
			pstmt.executeUpdate();
